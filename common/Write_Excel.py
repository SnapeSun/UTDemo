...
def copy_excel(case_path, report_path):
    """为了不污染用例数据，复制测试用例到report_path"""
...
class excel_write_result(object):
    """修改excel数据"""
...
if __name__ == "__main__":
    my_test_path = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))+'\\data\my_test.xlsx'
    data_path = os.path.abspath(os.path.dirname(os.path.dirname(__file__))) + '\\data\Interface_case.xlsx'
    print(my_test_path)

    copy_excel(data_path, my_test_path)

    wt = excel_write_result(my_test_path)
    wt.write(4, 5, "Hello")
    wt.write(4, 6, "World")
